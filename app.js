...
async function renderAgents(){
  ...
  const c=$('agents-list');
  c.innerHTML=agents.length===0?'<p style="text-align:center;color:var(--sub);padding:2rem;">No agents registered.</p>':agents.map(a=>{
    ...
    return`<div class="deal" style="border-left:3px solid var(--brand);">
      ...
      <div class="dm">📱 ${a.phone} · Rate: ${a.commission||20}%</div>
      <div class="dm" style="color:var(--text);">Earned: ${fmt(earned)} · Paid: ${fmt(paid)}</div>
    </div>`;
  }).join('');
  $('agent-perf-body').innerHTML=agents.map(a=>{
    const d=deals.filter(x=>x.agent?.name===a.name).length;
    const c=ledger.filter(l=>l.agent===a.name).reduce((s,l)=>s+(l.amount||0),0);
    return`<tr><td>${esc(a.name)}</td><td style="font-family:'JetBrains Mono',monospace;font-size:0.75rem;">${a.phone}</td><td>${d}</td><td style="color:var(--money);font-weight:700;">${fmt(c)}</td><td><span class="chip ca" style="position:static;">Active</span></td></tr>`;
  }).join('');
}

async function saveAgent(){
  const name=$('ag-name').value.trim();
  const phone=$('ag-phone').value.trim().replace(/\D/g,'');
  const rate=parseFloat($('ag-rate').value)||20;
  if(!name||!phone)return alert('Name and phone required.');
  SQ.push({t:'addAgent',d:{name,phone,commission:rate,joinedAt:new Date()}});
  closeM('add-agent-modal');
  $('ag-name').value='';$('ag-phone').value='';$('ag-rate').value='20';
  await new Promise(r=>setTimeout(r,600));
  await renderAgents();
  renderDashboard();
  log(`👤 Added agent: ${name}`);
}
